...
select nhacungcap.tennhacc, count(dangkycungcap.madkcc) 
from dangkycungcap
join nhacungcap on dangkycungcap.MaNhaCC = nhacungcap.MaNhaCC
where ngaybatdaucungcap = '2015/11/20'
group by dangkycungcap.madkcc;

select distinct hangxe from dongxe;

select MaDKCC, dangkycungcap.MaNhaCC, nhacungcap.TenNhaCC, nhacungcap.DiaChi, nhacungcap.MaSoThue, loaidichvu.TenLoaiDV, mucphi.DonGia, dongxe.HangXe, NgayBatDauCungCap, NgayKetThucCungCap
from dangkycungcap
join nhacungcap on dangkycungcap.manhacc = nhacungcap.manhacc
join dongxe on dangkycungcap.dongxe = dongxe.dongxe
join mucphi on dangkycungcap.mamp = mucphi.mamp
